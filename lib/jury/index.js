...
Jury.prototype.use = function(rules){
    if(!this._rules.hasOwnProperty(rules))  {
        console.log('Rules '+rules+' not found');
        return null;
    } 
    return new Case(this._rules[rules]);
}


function Case(rules){
    this._rules = rules;
    console.log('New Case Using');
}

Case.prototype.load = function(currentRequest)
{
    this._currentRequest = currentRequest;
    return this;
}

Case.prototype.judge = function(){
    console.log(this._rules);
    for (var index = 0; index < this._rules.length; index++) {
        this._rules[index].run('',this._currentRequest);        
    }
