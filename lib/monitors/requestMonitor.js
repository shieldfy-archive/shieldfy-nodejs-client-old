        ...
        return function (event, req, res) { 
           ...
           // var result = Client._jury.use('request').judge(); // csrf all file uploads
            ...
            if (event === 'request') {
                // console.log('====================================');
                // console.log('********************************************************');
                // console.log(Client._currentRequest._$req);
                // console.log('====================================');
                try {
                    new requestCollector(Client._currentRequest._$req,null,function(data){
                        Client._monitorBase.getReq(data)
                        var body = data.getInfo().body
                        var querys = data.getInfo().query
                        var args = Object.assign(body, querys)
                        for (var arg in args) {
                            try {
                                var rr = Client._jury.use('request').judge(args[arg]);
                                if (rr) {
                        ...
                        // userMonitor -> data._user.setScore(100)
