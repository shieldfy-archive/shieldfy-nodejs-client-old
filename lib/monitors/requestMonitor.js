    ...
    shimmer.wrap(exports && exports.Server && exports.Server.prototype, 'emit', function (original) {
        return function (event, req, res) {  
            ...
            if (event === 'request') {
                ...
                //Client._jury.use('request');//.judge();
                
                //var result = Client._jury.use('request').load(Client._currentRequest).judge(); // csrf all file uploads
                //Client._jury.use('request').target('$req.headers.host').judge(req.headers.origin);
